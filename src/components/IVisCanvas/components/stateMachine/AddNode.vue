<template>
  ...
  >
    ...
    >
      ...
      <el-form-item prop="shape" label="节点形状">
        <el-select
          v-model="form.shape"
          style="width: 100%"
          placeholder="请选择"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        shape: [{ required: true, message: "请选择节点形状", trigger: "blur" }],
      },
    };
  },
  computed: {
    title() {
      return (this.optionType !== 1 ? "状态机" + "编辑" : "新增") + "节点";
    },
    shapeDs() {
      return [
        { name: "圆", code: "circle" },
        { name: "正方形", code: "square" },
  ...
  methods: {
    ...
    initEdit(data) {
      data.color =
        data.color && data.color.background ? data.color.background : null;
