    ...
    >
      <el-form ref="addForm" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="旧密码：" prop="oldPassword" label-width="120px">
          <el-input v-model.trim="form.oldPassword" maxlength="20" placeholder="请输入旧密码" show-password />
        </el-form-item>
        <el-form-item label="新密码：" prop="password" label-width="120px">
          <el-input
            v-model.trim="form.password"
            maxlength="20"
            placeholder="请输入新密码,长度6-18位,必须包含数字和大小写字母"
            show-password
          />
        </el-form-item>
      ...
      <div class="text-center pt-2">
        <el-button type="primary" @click="submit('addForm')"><span class="mx-2">提交</span></el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  data() {
    var validPassword = (rule, value, callback) => {
      ...
      if (value == null || value.length < 6 || value.length > 18) {
        return callback(new Error('请输入长度6-18位,必须包含数字和大小写字母的密码'))
      }
    ...
    return {
      ...
      rules: {
        ...
        password: [
          {
            required: true,
            message: '请输入新密码,长度6-18位,必须包含数字和大小写字母',
            trigger: 'blur',
          },
